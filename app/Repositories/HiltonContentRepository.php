...
class HiltonContentRepository
{
    ...
    public static function getIdsByGiataPlace(string $place): array
    {
        ...
        $expedia_id = Mapping::hilton()->whereIn('giata_id', $tticodes)
            ->select('supplier_id')
            ->get()
            ->pluck('supplier_id')
            ->toArray();
    ...
    {
        $expedia_id = Mapping::hilton()->whereIn('giata_id', $giataIds)
            ->select('supplier_id')
            ->get()
            ->pluck('supplier_id')
            ->toArray();
    ...
    public static function getIdsByDestinationGiata(string $input): array
    {
        ...
        return $query->leftJoin($mainDB.'.mappings', $mainDB.'.mappings.giata_id', '=', 'properties.code')
            ...
            ->where($mainDB.'.mappings.supplier', MappingSuppliersEnum::HILTON->value)
            ->get()
            ->pluck('supplier_id')
            ->toArray();
    }
    ...
    public static function getDetailByGiataId($giata_id): object
    {
        ...
        return HiltonProperty::where('prop_code', function ($query) use ($giata_id, $mainDB) {
    ...
    {
        return HiltonProperty::where('prop_code', $hotel_id)
            ->select('name')
            ->first()
            ->name;
    }
