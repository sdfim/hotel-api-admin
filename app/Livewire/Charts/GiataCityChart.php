            ...
            $queryResult = DB::select("
                ...
                FROM
                    (
                        ...
                        FROM " . config('database.connections.mysql2.database') . "." . "giata_properties
                        GROUP BY city_id
                        ORDER BY count DESC
                        LIMIT 10
                    ) AS gp
                LEFT JOIN
                    " . config('database.connections.mysql2.database') . "." . "giata_geographies gg ON gp.city_id = gg.city_id");
