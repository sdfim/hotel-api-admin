                ...
                ActionGroup::make([
                    ...
                    DeleteAction::make()
                        ...
                        ->modalDescription(function ($record) {
                            $hotelRoomNames = HotelRoom::whereJsonContains('bed_groups', $record->name)
                                ->pluck('name')
                                ->unique()
                                ->toArray();
                            if (count($hotelRoomNames) > 0) {
                                return 'Warning:\nRoom bed type "' . $record->name . '" is used by hotel rooms: ' . implode(', ', $hotelRoomNames);
                            }
                            return 'This action will permanently delete the selected room bed type.';
                        })
