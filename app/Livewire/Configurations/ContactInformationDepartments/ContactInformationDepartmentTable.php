                ...
                ActionGroup::make([
                    ...
                    DeleteAction::make()
                        ...
                        ->modalDescription(function ($record) {
                            ...
                            $emails = ContactInformationEmails::whereJsonContains('departments', $record->name)
                                ->with('information')
                                ->get();
                            
                            $contactNames = $emails->map(function ($email) {
                                return $email->information->first_name . ' ' . $email->information->last_name;
                            })
                            ->filter()
                            ->unique()
                            ->toArray();
                            ...
                            if (count($contactNames) > 0) {
                                $warnings[] = 'Department "' . $record->name . '" is used by contacts: ' . implode(', ', $contactNames);
                            }
                            
                            if (count($warnings) > 0) {
                                return "Warning:\n" . implode("\n", $warnings);
                            }
                            return 'This action will permanently delete the selected department.';
                        })
