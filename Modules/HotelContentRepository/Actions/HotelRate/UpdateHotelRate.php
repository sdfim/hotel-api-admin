...
class UpdateHotelRate
{
    public function execute(HotelRate $hotelRate, array $data): void
    {
        ...
        if (! empty($validRoomIds)) {
            ...
            $hotelRate->productDescriptiveContentSections->each(function ($descriptiveContent) use ($validRoomIds) {
                $priorityRooms = is_array($descriptiveContent->priority_rooms) ? $descriptiveContent->priority_rooms : [];
                $filteredRooms = array_values(array_intersect($priorityRooms, $validRoomIds));
                $descriptiveContent->update(['priority_rooms' => $filteredRooms]);
            });
        }
    }
}
