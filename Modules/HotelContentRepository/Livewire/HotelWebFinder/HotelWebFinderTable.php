        ...
        if ($recordId) {
            ...
            if ($webFinder) {
                ...
                $data['units'] = $webFinder->units->map(function ($unit) {
                    ...
                    if (in_array($unit->field, ['search_start_travel_date_name', 'search_end_travel_date_name'])) {
                        $unitData['type_select'] = $unit->type;
                    } elseif ($unit->field === 'search_property_identifier_name') {
                        $unitData['type_text'] = $unit->type;
                    }
                    return $unitData;
                })->toArray();
            }
    ...
    protected function updateFinder(?HotelWebFinder $record = null): string
    {
        ...
        if (! empty($this->units)) {
            ...
            foreach ($this->units as $unit) {
                $unit['type'] = $unit['type_select'] ?? $unit['type_text'] ?? null;
                unset($unit['type_select'], $unit['type_text']);
