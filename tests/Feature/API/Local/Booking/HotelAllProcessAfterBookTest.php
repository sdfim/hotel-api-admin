...
test('book method response', function () {
    // # SEARCH 1

    // step 1 Search endpoint api/pricing/search
    $jsonData = searchRequest();
 ...
 * @return array[]
 */
function changeBookingRequest(): array
{
    return [
        'query' => [
            'given_name' => 'John',
...
function searchRequestStep2(): array
{
    $checkin = Carbon::now()->addDays(7)->toDateString();
    $checkout = Carbon::now()->addDays(7 + rand(2, 5))->toDateString();
 ...
 * @return array[]
 */
function addPassengersRequest(string $bookingId, array $bookingItems, array $occupancy): array
{
