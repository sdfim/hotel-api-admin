...
return new class extends Migration
{
    public function up()
    {
        Schema::create('pd_hotel_contact_information_job_descriptions', function (Blueprint $table) {
            $table->foreignId('job_descriptions_id')
                ->constrained('config_job_descriptions')
                ->onDelete('cascade')
                ->name('fk_job_desc_id');
            $table->foreignId('contact_information_id')
                ->constrained('pd_hotel_contact_information')
                ->onDelete('cascade')
    ...
    {
        Schema::dropIfExists('pd_hotel_contact_information_job_descriptions');
    }
};
