...
set -f
PATH_SSL="./infrastructure/docker/php-8.2/ssl"

# Path to the custom Homestead $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.travelagentadmin.cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.travelagentadmin.crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.travelagentadmin.key"
...
BASE_CNF="
    [ ca ]
    default_ca = ca_travelagentadmin
...
then
    ...
    openssl x509 -req -extfile "$PATH_CNF" \
        -extensions server_cert -days 3650 -sha256 \
        -in "$PATH_CSR" \
