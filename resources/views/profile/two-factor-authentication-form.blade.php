<x-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600 dark:text-zinc-100">
            <p>
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-grey-100">
                ...
                <div class="mt-4 max-w-xl text-sm text-gray-600 dark:text-zinc-100">
                    <p class="font-semibold">
        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-button type="button" class="dark:bg-gray-600" wire:loading.attr="disabled">
                        {{ __('Enable') }}
                    </x-button>
                </x-confirms-password>
            @else
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-secondary-button class="mr-3 dark:bg-gray-600 dark:text-gray-100 dark:border-transparent">
                            {{ __('Regenerate Recovery Codes') }}
                        </x-secondary-button>
                    </x-confirms-password>
                @elseif ($showingConfirmation)
                    <x-confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-button type="button" class="mr-3" wire:loading.attr="disabled" class="dark:bg-gray-600">
                            {{ __('Confirm') }}
                        </x-button>
                    </x-confirms-password>
                @else
