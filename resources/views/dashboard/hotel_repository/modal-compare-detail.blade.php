@php
    ...
    $hotelData = $hotelService->getDetailData($giataCode);
    ...
    @foreach ($providers as $provider)
        ...
        <div x-show="activeTab === '{{ $provider }}'" class="p-4 border rounded-lg shadow-sm bg-white">
            ...
            @foreach ($hotelData[$provider] as $section => $items)
                <div class="mb-4 p-3  bg-gray-50/50">
                    @if (is_array($items))
                        <h3 class="text-2lg font-medium text-gray-700">{{ ucfirst(str_replace('_', ' ', $section)) }}</h3>
                        <ul class="list-disc list-inside mt-2 text-gray-600">
                            @include('dashboard.hotel_repository.modal-compare-recursive-list', ['data' => $items, 'level' => 1, 'section' => $section])                        </ul>
                    @else
                        <div class="inline-flex items-center space-x-2">
                            <h3 class="text-2lg font-medium text-gray-700">
                                {{ ucfirst(str_replace('_', ' ', $section)) }}
                            </h3>
                            <p class="text-lg text-gray-600">{!! $items !!}</p>
                        </div>
                    @endif
                </div>
            @endforeach
