...
@section('content')
    ...
    <h2 class="font-semibold"
        x-data="{ message: '{{ $vendor->exists ? $text['edit'] : $text['create'] }} {{ $vendor->name }}' }"
        x-text="message"></h2>
            ...
            <div x-data="{activeTab: '{{ $tabGroups[array_key_first($tabGroups)]['tab_name'] }}' }"
                ...
                <ul class="sr_tab-list flex justify-between w-full">
                    @foreach ($tabGroups as $group => $tabs)
                                        @php if (!$vendor->exists && in_array($group, $ignoreCreate))
                                        continue; @endphp
                                        @php if ($vendor->products->count() && in_array($group, $nonProductTitle))
                                        continue; @endphp
                                        <li class="sr_tab-item mr-1 flex items-end">
                                            <a href="#" class="sr_tab-link" :class="{ 'sr_active': activeTab === '{{ $tabs['tab_name'] }}' }"
                                                @click.prevent="
