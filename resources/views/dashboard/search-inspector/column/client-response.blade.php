<button type="button" onClick="openModal('{{ $getRecord()->id }}','client-response')"
	class="text-white bg-green-500 border-green-500 btn hover:bg-green-600 focus:ring ring-green-200 focus:bg-green-600"
	data-tw-target="#modal-idlargemodal-{{ $getRecord()->id }}-client-response">JSON</button>
<div class="relative z-50 hidden modal" id="modal-idlargemodal-{{ $getRecord()->id }}-client-response"
				...
				<div class="bg-white dark:bg-zinc-700">
					<div class="flex items-center p-4 border-b rounded-t border-gray-50 dark:border-zinc-600">
						<h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100 ">
							{{ $getRecord()->id }}
							{{ $getRecord()->client_response_path }}
						</h3>
						<button onClick="closeModal('{{ $getRecord()->id }}','client-response')"
					...
					<div class="modal-body">
						Response:
						@php
						$file = Storage::get($getRecord()->client_response_path);
						@endphp
					...
					<div
						...
						<button type="button"
							...
							onClick="closeModal('{{ $getRecord()->id }}','client-response')">Cancel</button>
					</div>
				</div>
			</div>
		</div>
