<button type="button" onClick="openModal('{{ $getRecord()->booking_item }}','pricing-data')"
        class="text-white bg-green-500 border-green-500 btn hover:bg-green-600 focus:ring ring-green-200 focus:bg-green-600"
        data-tw-target="#modal-idlargemodal-{{ $getRecord()->booking_item }}-pricing-data">Pricing Data
</button>
<div class="relative z-50 hidden modal" id="modal-idlargemodal-{{ $getRecord()->booking_item }}-pricing-data"
                ...
                <div class="bg-white dark:bg-zinc-700">
                    <div class="flex items-center p-4 border-b rounded-t border-gray-50 dark:border-zinc-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100 ">
                            {{ $getRecord()->booking_item }}
                        </h3>
                        <button onClick="closeModal('{{ $getRecord()->booking_item }}','pricing-data')"
                    ...
                    <div class="modal-body">
                        Booking pricing data:
                        <pre>{{json_encode(json_decode($getRecord()->booking_pricing_data), JSON_PRETTY_PRINT) }}</pre>
                    </div>
                    <!-- Modal footer -->
                    <div
                        ...
                        <button type="button"
                                ...
                                onClick="closeModal('{{ $getRecord()->booking_item }}','pricing-data')">Cancel
                        </button>
                    </div>
                </div>
            </div>
