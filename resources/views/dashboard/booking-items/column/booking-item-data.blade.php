<button type="button" onClick="openModal('{{ $getRecord()->booking_item }}','item-data')"
        class="text-white bg-green-500 border-green-500 btn hover:bg-green-600 focus:ring ring-green-200 focus:bg-green-600"
        data-tw-target="#modal-idlargemodal-{{ $getRecord()->booking_item }}-item-data">Item data
</button>
<div class="relative z-50 hidden modal" id="modal-idlargemodal-{{ $getRecord()->booking_item }}-item-data"
            ...
            <div
                ...
                <div class="bg-white dark:bg-zinc-700">
                    <div class="flex items-center p-4 border-b rounded-t border-gray-50 dark:border-zinc-600">
                        ...
                        <button onClick="closeModal('{{ $getRecord()->booking_item }}','item-data')"
                    ...
                    <div class="modal-body text-slate-900 dark:text-white mt-5 text-base font-medium tracking-tight">
                        Booking item data:
                        <pre>{{json_encode(json_decode($getRecord()->booking_item_data), JSON_PRETTY_PRINT) }}</pre>
                    </div>
                    <!-- Modal footer -->
                    <div
                        ...
                        <button type="button"
                                ...
                                onClick="closeModal('{{ $getRecord()->booking_item }}','item-data')">Cancel
                        </button>
                    </div>
                </div>
            </div>
