<button type="button" onClick="openModal('{{ $getRecord()->id }}','response')"
	class="text-white bg-green-500 border-green-500 btn hover:bg-green-600 focus:ring ring-green-200 focus:bg-green-600"
	data-tw-target="#modal-idlargemodal-{{ $getRecord()->id }}-response">JSON</button>
<div class="relative z-50 hidden modal" id="modal-idlargemodal-{{ $getRecord()->id }}-response"
				...
				<div class="bg-white dark:bg-zinc-700">
					...
					<div
						...
						<button type="button"
							...
							onClick="closeModal('{{ $getRecord()->id }}','response')">Cancel</button>
					</div>
				</div>
			</div>
		</div>
